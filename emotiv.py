...
class Emotiv(object):
    ...
    def setup_not_windows(self):
        ...
        try:
            hidraw = self.open_hid_onmac()

        except Exception as ex:
            print ("Error opening usb dongle:" + ex.message)
            self.running = False
            return
        ...
        while self.running:
            try:
                if _os_decryption:
                    # This is actually not being used at all..
                    data = hidraw.read(32)
                    if data != "":
                    ...
                    if len(data)>0:
                        ...
                        if _os_decryption:
                            ...
                            # Queue it!
                            #print ("Queing for encryption...")
                            tasks.put_nowait(''.join(map(chr, data[1:])))
                            self.packets_received += 1
                    else:
                        if (self.readcounter==0 and iterations>50):
                            self.running = False
                gevent.sleep(DEVICE_POLL_INTERVAL)
            except KeyboardInterrupt:
                self.running = False
    ...
    def update_console(self):
        ...
        if self.display_output:
            while self.running:
                ...
                print "Queue Size: %i" % self.packets.qsize()
                gevent.sleep(DEVICE_POLL_INTERVAL)
